...
import UserFileSheetController, {
  UserFileSheetControllerRef,
} from "@/components/users/user/sheets/UserFileSheetController";
import { fileCategoryToSheetMode } from "@/components/users/user/types";
...
const User = () => {
  ...
  const fileSheetRef = useRef<UserFileSheetControllerRef>(null);
  ...
  const handleCreateForCategory = useCallback(
    (category: FileCategoryKey) => {
      ...
      const mode = fileCategoryToSheetMode[category];

      if (!mode) return;

      if (mode === "evaluation") {
        fileSheetRef.current?.openEvaluation(null);
        return;
      }

      if (mode === "jsaSelection") {
        fileSheetRef.current?.openJsaSelection();
        return;
      }

      if (mode === "workHardening") {
        fileSheetRef.current?.openWorkHardening();
        return;
      }

      if (mode === "newHire") {
        fileSheetRef.current?.openNewHire();
      }
    },
    [closeOpenSwipeable],
  );
  ...
  const handleOpenEvaluation = useCallback(
    (evaluationId: string) => {
      closeOpenSwipeable();
      fileSheetRef.current?.openEvaluation(evaluationId);
    },
    [closeOpenSwipeable],
  );
